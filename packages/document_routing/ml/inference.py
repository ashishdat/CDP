...
class MLEligibilityInference:
    def __init__(self,artifact_dir:Path):
      ...
      if self.metadata["model_type"]=="lightgbm":
        ...
        self.model=xgb.Booster();self.model.load_model(model_path)
    ...
    def predict(self,features:MLEligibilityFeatures)->tuple[list[MLRouteEvidence],float]:
      import numpy as np
      start=time.perf_counter();x=np.array([[getattr(features,k) for k in FEATURE_NAMES]],dtype=float)
      if self.metadata["model_type"]=="xgboost":
        import xgboost as xgb
        probs=self.model.predict(xgb.DMatrix(x,feature_names=FEATURE_NAMES))[0]
