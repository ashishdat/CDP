...
from evaluation.generate_public_synthetic_claims import (
    _fields, _intersects, _render, _template, _valid_npi, _value_layout,
)
...
def test_field_retry_is_structured_and_npi_always_gets_independent_read():
    assert _should_retry_field("npi", 0.99)
    assert not _should_retry_field("date", 0.50)
...
def test_synthetic_labels_never_overlap_populated_data_regions():
    ...
    red_pixels = sum(
        count for count, (r, g, b) in interior.getcolors(maxcolors=interior.width * interior.height)
...
def test_benchmark_can_route_region_only_engines_without_using_full_page_ocr():
    ...
    class RegionOnly:
        def extract_region(self, image, x0, y0, x1, y1):
            calls.append((x0, y0, x1, y1)); return []
