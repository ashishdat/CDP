...
def charge_locals_settled(candidates: list[Mapping[str, Any]] | None) -> bool:
    ...
    return any(len(groups) >= 2 for groups in by_amt.values())
...
def name_locals_settled(candidates: list[Mapping[str, Any]] | None) -> bool:
    ...
    if len(shaped) < 2:
        return False
    # Group soft-equivalent names; require ≥2 families in one clique.
...
def should_skip_all_cloud(
    ...
    if key in {"total_charge", "total_charges", "charges", "charge_amount"}:
        return charge_locals_settled(cands)
