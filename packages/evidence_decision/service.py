...
class EvidenceDecisionService:
    ...
    def __init__(self, reconciler: EvidenceReconciler | None = None,
                 ...
                 route_registry: RouteRegistry | None = None) -> None:
    ...
    def decide(self, context: DecisionContext) -> FieldDecision:
        ...
        if route is not None and route not in self.route_registry.routes_for_mode(self.route_mode):
            ...
            if len(eligible) != len(candidates):
                ...
                route_reasons.append(
                    f"ROUTE_STATUS_REJECTED:{route.route_id}:{route.status.value}"
                )
        bundle = build_evidence_bundle(
            field_name=context.field_name, candidates=candidates,
        ...
        field_policy = self.field_policy.for_field(context.document_family, context.field_name)
        ...
        if context.requires_review_when_unresolved is None:
            review_unresolved = (
                False if context.blocks_stp is False
        ...
        policy_satisfied, available, missing, gap_reasons = self.evidence_policy.evaluate(
            context.field_name, context.criticality, bundle, context.document_family,
            reference_authorized=(context.reference_source_state is ReferenceSourceState.AUTHORIZED),
        )
        policy_id, _ = self.evidence_policy.field_spec(
            context.document_family, context.field_name, context.criticality,
        )
        ...
        result = self.reconciler.reconcile(
            context.field_name, candidates, context.criticality,
        ...
        if not context.hard_validation_passed:
            if not candidates or not any(candidate.value for candidate in candidates):
                ...
                reasons = [
                    *result.rationale_codes, "INVALID_FORMAT", *gap_reasons,
                    *route_reasons, *next_reasons,
                ]
            ...
            reasons = [*result.rationale_codes, "CALIBRATION_OR_AMBIGUITY_REQUIRES_REVIEW", *route_reasons]
        elif result.decision is Decision.ESCALATE:
            action, next_reasons = self._next_action(context, available, missing)
            disposition = (FieldDisposition.ESCALATE if action is not NextAction.HUMAN_REVIEW
        ...
        return FieldDecision(
            field_id=context.field_id, field_name=context.field_name,
    ...
    def _next_action(self, context: DecisionContext, available, missing) -> tuple[NextAction, list[str]]:
        requirements = tuple(
            frozenset(item.value for item in option)
            for option in self.evidence_policy.requirements(
                context.field_name, context.criticality, context.document_family,
            )
        )
        ...
        decision = self.gap_router.route(
            available=set(available), requirements=requirements,
    ...
    def _terminal(
        ...
        *, bundle=None, available=(), missing=(),
        ...
        field_policy = self.field_policy.for_field(
            context.document_family, context.field_name,
        )
        return FieldDecision(
            field_id=context.field_id, field_name=context.field_name,
