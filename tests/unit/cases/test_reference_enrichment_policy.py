...
def request(attributes: dict[str, str], field: str = "patient_last") -> ReferenceLookupRequest:
    return ReferenceLookupRequest(request_id="r1", identity_key="D|1|CMS1500||patient_last",
        document_id="D", page_number=1, document_family="CMS1500", field_name=field,
        criticality="CRITICAL", current_candidate="OCR", available_claim_attributes=attributes,
        requested_at=datetime.now(UTC), policy_version="v1")


def record(attributes: dict[str, str], *, lineage: list[str] | None = None,
           authorized: bool = True, status: str = "ACTIVE") -> ReferenceRecord:
    return ReferenceRecord(provider_name="member-master", provider_type="MEMBER",
        provider_authorized=authorized, dataset_version="2026-08", source_record_id="M1",
        source_lineage=lineage or ["member-master"], independent_truth=True,
        non_circular_lineage=True, reference_attributes=attributes,
        field_values={"patient_last": "Smith"}, record_status=status, response_hash="abc")
...
def test_name_only_and_multiple_matches_fail_closed() -> None:
    ...
    assert decide(request(name), [record(name), record(name)]).decision == "MULTIPLE_REFERENCE_MATCHES"
...
def test_circular_lineage_and_unauthorized_provider_are_rejected() -> None:
    ...
    assert decide(request(attrs), [record(attrs, authorized=False)]).decision == "PROVIDER_UNAUTHORIZED"
...
def test_finalized_independent_downstream_requires_verified_mapping() -> None:
    ...
    downstream = record(attrs, status="FINALIZED").model_copy(update={"provider_type": "FINALIZED_CLAIMS"})
    assert decide(request(attrs), [downstream]).decision == "DOWNSTREAM_VERIFIED"
    unverified = downstream.model_copy(update={"reference_attributes": {**attrs, "field_mapping_verified": "false"}})
...
def test_critical_correction_requires_distinct_second_approver() -> None:
    attrs = {"member_id": "M1", "dob": "1980-01-02", "name": "Jane Smith",
        "primary_approved_by": "reviewer-1", "primary_approved_at": "2026-08-01T10:00:00+00:00",
        "second_approved_by": "reviewer-1", "second_approved_at": "2026-08-01T11:00:00+00:00",
    ...
    distinct = correction.model_copy(update={"reference_attributes": {**attrs, "second_approved_by": "reviewer-2"}})
