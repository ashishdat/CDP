...
class DeterministicHierarchicalBaseline:
    ...
    def classify(self, document_id: str, page_id: str,
                 evidence: HierarchicalRoutingEvidence) -> DocumentClassification:
        route = evidence.legacy_route
        if route == MultiSignalRoute.NON_CLAIM:
            top, family, subtype = DocumentClass.NON_CLAIM, DocumentClass.NON_CLAIM, DocumentClass.OTHER_NON_CLAIM
        ...
        return DocumentClassification(document_id=document_id, page_id=page_id,
            top_level_class=top, document_family=family, document_subtype=subtype,
            structured=evidence.structured, claim_related=evidence.claim_related,
            standard_candidate=route in {MultiSignalRoute.CMS1500, MultiSignalRoute.UB04},
            confidence=evidence.confidence, supporting_evidence=evidence.supporting_codes,
            contradicting_evidence=evidence.contradicting_codes,
            ambiguity_reason=("LEGACY_ROUTE_DOES_NOT_RESOLVE_TAXONOMY" if top == DocumentClass.UNKNOWN else None),
