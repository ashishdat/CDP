...
class EvidencePolicy:
    ...
    def load(cls, path: str | Path = DEFAULT_EVIDENCE_POLICY_PATH) -> EvidencePolicy:
        try:
            ...
            raise EvidencePolicyUnavailableError(
                f"evidence policy unavailable: {path}"
            ) from exc
        if not isinstance(payload, dict) or "version" not in payload:
            raise EvidencePolicyUnavailableError("evidence policy is malformed")
        return cls(payload)

    def field_spec(self, document_family: str, field_name: str, criticality: CriticalityLevel) -> tuple[str, dict]:
    ...
    def requirements(self, field_name: str, criticality: CriticalityLevel,
                     document_family: str = "*") -> tuple[frozenset[EvidenceClass], ...]:
    ...
    def evaluate(self, field_name: str, criticality: CriticalityLevel,
                 bundle: EvidenceBundle, document_family: str = "*",
                 reference_authorized: bool = False) -> tuple[bool, tuple[str, ...], tuple[str, ...], tuple[str, ...]]:
        ...
        eligible = tuple(
            ...
            if EvidenceClass.E5 not in option or EvidenceClass.E5 in available or reference_authorized
        ) or options
