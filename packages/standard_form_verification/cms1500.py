...
class CMS1500Verifier:
    ...
    def verify(self, evidence: StandardFormEvidence) -> StandardFormVerification:
        ...
        classes = {
            ...
            "PATIENT_INSURED_RELATIONSHIP": regions.get("patient_insured", 0) >= 0.55,
            "CLAIM_DIAGNOSIS_LAYOUT": min(
                regions.get("claim_information", 0), regions.get("diagnosis", 0)
            )
            >= 0.45,
            "SERVICE_LINE_GRID": evidence.service_grid_score >= 0.65,
            "PROVIDER_BILLING_LAYOUT": regions.get("provider_billing", 0) >= 0.45,
        ...
        return _result(
            evidence,
            classes,
            essential=("CANONICAL_FORM_IDENTITY", "SERVICE_LINE_GRID", "SPATIAL_RELATIONSHIPS"),
...
def _result(evidence, classes, essential, policy):
    ...
    status = (
        ...
        if verified
            ...
            StandardFormStatus.AMBIGUOUS
            if len(support) >= 2 and not contradictions
