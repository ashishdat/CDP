...
class DocumentRoutingDecisionService:
    ...
    def decide(self, document_id: str, page_id: str, routing_evidence: RoutingEvidence,
               standard_evidence: StandardFormEvidence | None = None,
               evaluation_only: bool = False) -> DocumentRoutingDecision:
        classification = self.classifier.classify(document_id, page_id, from_routing_evidence(routing_evidence))
        return self.decide_classification(classification, standard_evidence, evaluation_only=evaluation_only)

    def decide_classification(self, classification: DocumentClassification,
                              standard_evidence: StandardFormEvidence | None = None,
                              evaluation_only: bool = False) -> DocumentRoutingDecision:
        verification = None
        if classification.standard_candidate:
            ...
            if standard_evidence is None or standard_evidence.candidate_family != candidate:
                standard_evidence = StandardFormEvidence(candidate_family=candidate,
        ...
        return DocumentRoutingDecision(classification=classification,
            standard_verification=verification, processing_route=route.route,
            route_reason_codes=route.reason_codes, decision_service_version=self.version,
            evaluation_only=evaluation_only)

    def decide_nomination(self, *, document_id: str, page_id: str,
                          nominated_family: DocumentClass | None, structured: bool,
                          claim_related: bool, non_claim: bool, confidence: float,
        ...
        if non_claim:
            ...
            top, family, subtype = DocumentClass.CLAIM, DocumentClass.STANDARD_CLAIM, nominated_family
        else:
            top = family = subtype = DocumentClass.UNKNOWN
        classification = DocumentClassification(document_id=document_id, page_id=page_id,
            top_level_class=top, document_family=family, document_subtype=subtype,
            structured=structured, claim_related=claim_related,
            standard_candidate=nominated_family is not None, confidence=confidence,
            supporting_evidence=supporting_evidence,
            ambiguity_reason=("NO_VERIFIED_TAXONOMY_SUBTYPE" if top == DocumentClass.UNKNOWN else None),
